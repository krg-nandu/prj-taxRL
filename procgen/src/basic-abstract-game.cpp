...
BasicAbstractGame::BasicAbstractGame(std::string name)
    ...
    black_bg_image = std::make_shared<QImage>(500, 500, QImage::Format_RGB32);
    black_bg_image->fill(Qt::black);
}
...
void BasicAbstractGame::initialize_asset_if_necessary(int img_idx) {
    ...
    if (names.size() == 0) {
        ...
        // TODO: implement this for all games
        if (options.vision_mode == "semantic_mask" || options.vision_mode == "fg_mask")
            asset_ptr = segment_asset(asset_ptr, type, theme);
...
std::shared_ptr<QImage> BasicAbstractGame::segment_asset(std::shared_ptr<QImage> asset_ptr, int base_type, int theme) {
}
...
void BasicAbstractGame::color_asset(std::shared_ptr<QImage> asset, int r, int g, int b) {
    QRgb color = qRgb(r, g, b);
    bool pixel_might_be_transparent;
    QRgb *line;

    for (int y = 0; y < asset->height(); y++) {
        line = (QRgb *) asset->scanLine(y);
        for (int x = 0; x < asset->width(); x++) {
            /*
                use a heuristic that assumes true black pixels -- (0,0,0) RGB -- may be "transparent" pixels and
                should as such should not be touched (which would ruin their transparency).
            */
            pixel_might_be_transparent = qRed(line[x]) == 0 && qGreen(line[x]) == 0 && qBlue(line[x]) == 0;
            if (pixel_might_be_transparent) continue;
...
void BasicAbstractGame::draw_background(QPainter &p, const QRect &rect) {
    ...
    std::shared_ptr<QImage> background_image = semantic_mask || fg_mask ? black_bg_image : main_bg_images_ptr->at(background_index);
