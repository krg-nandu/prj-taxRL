...
const int CAR_COLOR = 50;
const int BACKGROUND_PLATFORM = TERRAIN_COLOR;
const int LOG_COLOR = 125;
const int ROAD_COLOR = 150;
const int WATER_COLOR = 175;

const float MONSTER_RADIUS = 0.25;
const float LOG_RADIUS = 0.45f;

const int GOAL_REWARD = 10.0f;
const int INTER_REWARD = 1.0f;
...
class LeaperHGame : public BasicAbstractGame {
  public:
    int bottom_road_y = 0;
    ...
    int goal_y = 0;
    std::vector<bool> road_lane_enter;
    std::vector<bool> water_lane_enter;
    ...
    std::shared_ptr<QImage> segment_asset(std::shared_ptr<QImage> asset_ptr, int base_type, int theme) override {
        if (options.vision_mode == "semantic_mask") {
            if (base_type == ROAD)
                color_asset(asset_ptr, ROAD_COLOR, ROAD_COLOR, ROAD_COLOR);
            else if (base_type == WATER)
                color_asset(asset_ptr, WATER_COLOR, WATER_COLOR, WATER_COLOR);
            else if (base_type == CAR)
                color_asset(asset_ptr, CAR_COLOR, CAR_COLOR, CAR_COLOR);
            else if (base_type == LOG)
                color_asset(asset_ptr, LOG_COLOR, LOG_COLOR, LOG_COLOR);
            ...
            if (base_type == ROAD || base_type == WATER)
                color_asset(asset_ptr, BACKGROUND_PLATFORM, BACKGROUND_PLATFORM, BACKGROUND_PLATFORM);
            else
    ...
    void game_reset() override {
        ...
        for (int lane = 0; lane < num_road_lanes; lane++) {
            ...
            road_lane_enter.push_back(false);
        }
        ...
        for (int lane = 0; lane < num_water_lanes; lane++) {
            ...
            water_lane_enter.push_back(false);
        }
        ...
        max_possible_score = GOAL_REWARD + INTER_REWARD * (num_road_lanes + num_water_lanes);
    ...
    void game_step() override {
        ...
        for (auto &m : entities) {
            ...
            if (m->type == CAR && has_collision(agent, m, margin)){
                bumped_into_car = true;
            }
        }
        ...
        if (standing_on_log) {
            ...
            // check if this is the first time you are entering that lane
            for (int lane = 0; lane < int(water_lane_speeds.size()); lane++){
                if (( agent->y >= bottom_water_y + lane) && ( agent->y <= bottom_water_y+lane+1) && water_lane_enter[lane] == false){
                    water_lane_enter[lane] = true;
                    step_data.reward += INTER_REWARD;
                }
            }
        }

        for (int lane = 0; lane < int(road_lane_speeds.size()); lane++){
            // entered the lane for the first time, and there is no collission?
            if (( agent->y >= bottom_road_y + lane ) && (agent->y <= bottom_road_y + lane + 1) && (road_lane_enter[lane] == false) && !bumped_into_car){
                road_lane_enter[lane] = true;
                step_data.reward += INTER_REWARD;
            }
        }
...
REGISTER_GAME(NAME, LeaperHGame);
