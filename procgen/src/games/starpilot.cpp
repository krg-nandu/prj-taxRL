...
const int BARRIER_COLOR = 50;
const int FLYER_COLOR = 75;
const int FAST_FLYER_COLOR = 85;
const int TURRET_COLOR = 150;
const int BULLET_PLAYER_COLOR = 155;
const int BULLET2_COLOR = 160;
const int BULLET3_COLOR = 165;
const int CLOUD_COLOR = 175;
...
class StarPilotGame : public BasicAbstractGame {
  public:
    ...
    std::shared_ptr<QImage> segment_asset(std::shared_ptr<QImage> asset_ptr, int base_type, int theme) override {
        std::shared_ptr<QImage> asset_copy = std::make_shared<QImage>(*asset_ptr);

        if (options.vision_mode == "semantic_mask") {
            if (base_type == PLAYER)
                color_asset(asset_copy, PLAYER_COLOR, PLAYER_COLOR, PLAYER_COLOR);
            else if (base_type == BULLET_PLAYER)
                color_asset(asset_copy, BULLET_PLAYER_COLOR, BULLET_PLAYER_COLOR, BULLET_PLAYER_COLOR);
            else if (base_type == BULLET2)
                color_asset(asset_copy, BULLET2_COLOR, BULLET2_COLOR, BULLET2_COLOR);
            else if (base_type == BULLET3)
                color_asset(asset_copy, BULLET3_COLOR, BULLET3_COLOR, BULLET3_COLOR);
            else if (base_type == FLYER)
                color_asset(asset_copy, FLYER_COLOR, FLYER_COLOR, FLYER_COLOR);
            else if (base_type == FAST_FLYER)
                color_asset(asset_copy, FAST_FLYER_COLOR, FAST_FLYER_COLOR, FAST_FLYER_COLOR);
            else if (base_type == METEOR)
                color_asset(asset_copy, BARRIER_COLOR, BARRIER_COLOR, BARRIER_COLOR);
            else if (base_type == CLOUD)
                color_asset(asset_copy, CLOUD_COLOR, CLOUD_COLOR, CLOUD_COLOR);
            else if (base_type == TURRET)
                color_asset(asset_copy, TURRET_COLOR, TURRET_COLOR, TURRET_COLOR);
            else if (base_type == FINISH_LINE)
                color_asset(asset_copy, GOAL_COLOR, GOAL_COLOR, GOAL_COLOR);
            else if (base_type == EXPLOSION || base_type == EXPLOSION2 || base_type == EXPLOSION3 || base_type == EXPLOSION4 || base_type == EXPLOSION5)
                color_asset(asset_copy, EXPLOSION_COLOR, EXPLOSION_COLOR, EXPLOSION_COLOR);
        } else if (options.vision_mode == "fg_mask") {
            color_asset(asset_copy, 255, 255, 255);
        }
    ...
    void game_draw(QPainter &p, const QRect &rect) override {
        ...
        // don't use background when segmenting
    ...
    void add_spawners() {
        ...
        for (int i = 0; t <= SHOOTER_WIN_TIME; i++) {
            ...
            for (int j = 0; j < group_size; j++) {
                ...
                if (is_destructible(type)) max_possible_score += ENEMY_REWARD;
