...
const int ENEMY_BULLET_COLOR = 75;
const int PLAYER_BULLET_COLOR = 150;
const int SHIELDS_COLOR = 175;
  ...
  public:
    ...
    std::shared_ptr<QImage> segment_asset(std::shared_ptr<QImage> asset_ptr, int base_type, int theme) override {
        ...
        if (options.vision_mode == "semantic_mask") {
            ...
            else if (base_type == BOSS)
                color_asset(asset_copy, GOAL_COLOR, GOAL_COLOR, GOAL_COLOR);
            else if (base_type == PLAYER_BULLET || base_type == REFLECTED_BULLET)
                color_asset(asset_copy, PLAYER_BULLET_COLOR, PLAYER_BULLET_COLOR, PLAYER_BULLET_COLOR);
            else if (base_type == ENEMY_BULLET)
                color_asset(asset_copy, ENEMY_BULLET_COLOR, ENEMY_BULLET_COLOR, ENEMY_BULLET_COLOR);
            else if (base_type == SHIELDS)
                color_asset(asset_copy, SHIELDS_COLOR, SHIELDS_COLOR, SHIELDS_COLOR);
            else if (base_type == BARRIER)
        ...
        if (src->type == PLAYER_BULLET) {
            ...
            if (target->type == SHIELDS) {
                if (shields_are_up) {
                    ...
                    src->expire_time = 4;
                    src->life_time = 0;
                    if (!(options.vision_mode == "semantic_mask" || options.vision_mode == "fg_mask")) src->alpha_decay = 0.8f; // no alpha decay when segmenting
                }
    ...
    void game_reset() override {
        ...
        max_possible_score = boss->health * POSITIVE_REWARD + COMPLETION_BONUS;
    ...
    void game_step() override {
        ...
        for (int i = (int)(entities.size()) - 1; i >= 0; i--) {
            ...
            // don't render bullet trails when segmenting
            /*
