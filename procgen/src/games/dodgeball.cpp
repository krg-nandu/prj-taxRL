...
const int DOOR_COLOR = 50;
const int ENEMY_COLOR = 75;
const int ENEMY_BALL_COLOR = 150;
const int DUST_CLOUD_COLOR = 175;
const int PLAYER_BALL_COLOR = 225;
  ...
  public:
    ...
    std::shared_ptr<QImage> segment_asset(std::shared_ptr<QImage> asset_ptr, int base_type, int theme) override {
        if (options.vision_mode == "semantic_mask") {
            ...
            else if (base_type == PLAYER_BALL)
                color_asset(asset_ptr, PLAYER_BALL_COLOR, PLAYER_BALL_COLOR, PLAYER_BALL_COLOR);
            else if (base_type == ENEMY)
                color_asset(asset_ptr, ENEMY_COLOR, ENEMY_COLOR, ENEMY_COLOR);
            else if (base_type == DOOR)
                color_asset(asset_ptr, DOOR_COLOR, DOOR_COLOR, DOOR_COLOR);
            else if (base_type == ENEMY_BALL)
                color_asset(asset_ptr, ENEMY_BALL_COLOR, ENEMY_BALL_COLOR, ENEMY_BALL_COLOR);
            else if (base_type == DOOR_OPEN)
                color_asset(asset_ptr, GOAL_COLOR, GOAL_COLOR, GOAL_COLOR);
            else if (base_type == LAVA_WALL || base_type == OOB_WALL)
                color_asset(asset_ptr, TERRAIN_COLOR, TERRAIN_COLOR, TERRAIN_COLOR);
            else if (base_type == DUST_CLOUD)
                color_asset(asset_ptr, DUST_CLOUD_COLOR, DUST_CLOUD_COLOR, DUST_CLOUD_COLOR);
    ...
    void game_reset() override {
        ...
        max_possible_score = num_enemies * ENEMY_REWARD + COMPLETION_BONUS;
