...
const int ENEMY_COLOR = 175;
const int SAW_COLOR = 150;
const int LAVA_COLOR = 125;
const int CRATE_COLOR = 75;
const int ENEMY_BARRIER_COLOR = 165;
  ...
  public:
    ...
    std::shared_ptr<QImage> segment_asset(std::shared_ptr<QImage> asset_ptr, int base_type, int theme) override {
        if (options.vision_mode == "semantic_mask") {
            if (base_type == PLAYER || base_type == PLAYER_JUMP || base_type == PLAYER_RIGHT1 || base_type == PLAYER_RIGHT2)
                color_asset(asset_ptr, PLAYER_COLOR, PLAYER_COLOR, PLAYER_COLOR);
            else if (base_type == ENEMY || base_type == ENEMY1 || base_type == ENEMY2)
                color_asset(asset_ptr, ENEMY_COLOR, ENEMY_COLOR, ENEMY_COLOR);
            else if (base_type == SAW || base_type == SAW2)
                color_asset(asset_ptr, SAW_COLOR, SAW_COLOR, SAW_COLOR);
            else if (base_type == GOAL)
                color_asset(asset_ptr, GOAL_COLOR, GOAL_COLOR, GOAL_COLOR);
            else if (base_type == WALL_MID || base_type == WALL_TOP)
                color_asset(asset_ptr, TERRAIN_COLOR, TERRAIN_COLOR, TERRAIN_COLOR);
            else if (base_type == LAVA_MID || base_type == LAVA_TOP)
                color_asset(asset_ptr, LAVA_COLOR, LAVA_COLOR, LAVA_COLOR);
            else if (base_type == ENEMY_BARRIER)
                color_asset(asset_ptr, ENEMY_BARRIER_COLOR, ENEMY_BARRIER_COLOR, ENEMY_BARRIER_COLOR);
            else if (base_type == CRATE)
                color_asset(asset_ptr, CRATE_COLOR, CRATE_COLOR, CRATE_COLOR);
    ...
    void game_reset() override {
        ...
        max_possible_score = GOAL_REWARD;
    ...
    void game_step() override {
        ...
        for (int i = (int)(entities.size()) - 1; i >= 0; i--) {
            ...
            if (ent->type == ENEMY) {
                // no alpha effects when segmenting
