...
const int MARKER_COLOR = 75;
const int ENEMY_EGG_COLOR = 150;
const int ORB_COLOR = 235;
const int ENEMY_WEAK_COLOR = 245;
const int LARGE_ORB_COLOR = GOAL_COLOR;
// ENEMY, ENEMY2, and ENEMY3 together form a flying animation.
// However, this flight animation effect is lost because repainting
// the asset for segmentation purposes causes a loss in transparency,
// so the enemy appears to have its "wings" spread in this animation
// at all times. Regardless, these states should all be the same color.
const int ENEMY_COLOR = 110;
  ...
  public:
    ...
    std::shared_ptr<QImage> segment_asset(std::shared_ptr<QImage> asset_ptr, int base_type, int theme) override {
        if (options.vision_mode == "semantic_mask") {
            ...
            else if (base_type == LARGE_ORB)
                color_asset(asset_ptr, LARGE_ORB_COLOR, LARGE_ORB_COLOR, LARGE_ORB_COLOR);
            else if (base_type == ENEMY_WEAK)
                color_asset(asset_ptr, ENEMY_WEAK_COLOR, ENEMY_WEAK_COLOR, ENEMY_WEAK_COLOR);
            else if (base_type == ENEMY_EGG)
                color_asset(asset_ptr, ENEMY_EGG_COLOR, ENEMY_EGG_COLOR, ENEMY_EGG_COLOR);
            else if (base_type == MAZE_WALL)
                color_asset(asset_ptr, TERRAIN_COLOR, TERRAIN_COLOR, TERRAIN_COLOR);
            else if (base_type == ENEMY || base_type == ENEMY2 || base_type == ENEMY3)
                color_asset(asset_ptr, ENEMY_COLOR, ENEMY_COLOR, ENEMY_COLOR);
            else if (base_type == MARKER)
                color_asset(asset_ptr, MARKER_COLOR, MARKER_COLOR, MARKER_COLOR);
    ...
    void draw_grid_obj(QPainter &p, const QRectF &rect, int type, int theme) override {
        if (type == ORB) {
            if (options.vision_mode == "semantic_mask" || options.vision_mode == "fg_mask")
                p.fillRect(QRectF(rect.x() + rect.width() * (1 - ORB_DIM) / 2, rect.y() + rect.height() * (1 - ORB_DIM) / 2, rect.width() * ORB_DIM, rect.height() * ORB_DIM), QColor(ORB_COLOR, ORB_COLOR, ORB_COLOR));
            else if (fg_mask)
                p.fillRect(QRectF(rect.x() + rect.width() * (1 - ORB_DIM) / 2, rect.y() + rect.height() * (1 - ORB_DIM) / 2, rect.width() * ORB_DIM, rect.height() * ORB_DIM), QColor(255, 255, 255));
            else
    ...
    void game_reset() override {
        ...
        max_possible_score = total_orbs * ORB_REWARD + COMPLETION_BONUS;
        orbs_collected = 0;
