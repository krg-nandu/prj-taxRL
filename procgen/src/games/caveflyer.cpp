...
const int OBSTACLE_COLOR = 50;
const int ENEMY_COLOR = 75;
const int PLAYER_BULLET_COLOR = 155;
const int TARGET_COLOR = 175;
  ...
  public:
    ...
    std::shared_ptr<QImage> segment_asset(std::shared_ptr<QImage> asset_ptr, int base_type, int theme) override {
        if (options.vision_mode == "semantic_mask") {
            ...
            else if (base_type == OBSTACLE)
                color_asset(asset_ptr, OBSTACLE_COLOR, OBSTACLE_COLOR, OBSTACLE_COLOR);
            else if (base_type == TARGET)
                color_asset(asset_ptr, TARGET_COLOR, TARGET_COLOR, TARGET_COLOR);
            else if (base_type == PLAYER_BULLET)
                color_asset(asset_ptr, PLAYER_BULLET_COLOR, PLAYER_BULLET_COLOR, PLAYER_BULLET_COLOR);
            else if (base_type == ENEMY)
    ...
    void game_reset() override {
        ...
        for (int i = 0; i < num_objs; i++) {
            ...
            if (i < chunk_size) {
                ...
                e->health = 5;
                e->collides_with_entities = true;
                max_possible_score += TARGET_REWARD;
            } else {
