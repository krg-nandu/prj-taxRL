...
const int OOB_WALL_COLOR = 75;
const int BOULDER_COLOR = 170;
const int ENEMY_COLOR = 125;
const int EXIT_COLOR = 240;
const int DIAMOND_COLOR = GOAL_COLOR;
  ...
  public:
    ...
    std::shared_ptr<QImage> segment_asset(std::shared_ptr<QImage> asset_ptr, int base_type, int theme) override {
        if (options.vision_mode == "semantic_mask") {
            ...
            else if (base_type == BOULDER || base_type == MOVING_BOULDER)
                color_asset(asset_ptr, BOULDER_COLOR, BOULDER_COLOR, BOULDER_COLOR);
            else if (base_type == DIAMOND || base_type == MOVING_DIAMOND)
                color_asset(asset_ptr, DIAMOND_COLOR, DIAMOND_COLOR, DIAMOND_COLOR);
            else if (base_type == EXIT)
                color_asset(asset_ptr, EXIT_COLOR, EXIT_COLOR, EXIT_COLOR);
            else if (base_type == DIRT)
                color_asset(asset_ptr, TERRAIN_COLOR, TERRAIN_COLOR, TERRAIN_COLOR);
            else if (base_type == OOB_WALL)
                color_asset(asset_ptr, OOB_WALL_COLOR, OOB_WALL_COLOR, OOB_WALL_COLOR);
    ...
    void game_reset() override {
        ...
        max_possible_score = num_diamonds * DIAMOND_REWARD + COMPLETION_BONUS;
