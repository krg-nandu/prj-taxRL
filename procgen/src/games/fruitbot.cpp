...
const int BAD_OBJ_COLOR = 50;
const int LOCKED_DOOR_COLOR = 75;
const int LOCK_COLOR = 85;
const int PLAYER_BULLET_COLOR = 150;
const int GOOD_OBJ_COLOR = 215;
  ...
  public:
    ...
    std::shared_ptr<QImage> segment_asset(std::shared_ptr<QImage> asset_ptr, int base_type, int theme) override {
        ...
        if (options.vision_mode == "semantic_mask") {
            ...
            else if (base_type == BARRIER || base_type == OUT_OF_BOUNDS_WALL)
                color_asset(asset_copy, TERRAIN_COLOR, TERRAIN_COLOR, TERRAIN_COLOR);
            else if (base_type == PLAYER_BULLET)
                color_asset(asset_copy, PLAYER_BULLET_COLOR, PLAYER_BULLET_COLOR, PLAYER_BULLET_COLOR);
            else if (base_type == BAD_OBJ)
                color_asset(asset_copy, BAD_OBJ_COLOR, BAD_OBJ_COLOR, BAD_OBJ_COLOR);
            else if (base_type == GOOD_OBJ)
                color_asset(asset_copy, GOOD_OBJ_COLOR, GOOD_OBJ_COLOR, GOOD_OBJ_COLOR);
            else if (base_type == LOCKED_DOOR)
                color_asset(asset_copy, LOCKED_DOOR_COLOR, LOCKED_DOOR_COLOR, LOCKED_DOOR_COLOR);
            else if (base_type == LOCK)
                color_asset(asset_copy, LOCK_COLOR, LOCK_COLOR, LOCK_COLOR);
            else if (base_type == PRESENT)
    ...
    void game_reset() override {
        ...
        min_possible_score = num_bad * PENALTY;
        max_possible_score = num_good * POSITIVE_REWARD + COMPLETION_BONUS;
