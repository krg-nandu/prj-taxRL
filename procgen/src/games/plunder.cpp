...
const int JUICE_COLOR = 25;
const int SHIP_0_COLOR = 30;
const int SHIP_1_COLOR = 40;
const int SHIP_2_COLOR = 50;
const int SHIP_3_COLOR = 60;
const int SHIP_4_COLOR = 70;
const int SHIP_5_COLOR = 80;
const int PANEL_COLOR = 75;
const int PROGRESS_COLOR = 125;
const int PLAYER_BULLET_COLOR = 150;
const int TARGET_BACKGROUND_COLOR = 225;
  ...
  public:
    ...
    std::shared_ptr<QImage> segment_asset(std::shared_ptr<QImage> asset_ptr, int base_type, int theme) override {
        if (options.vision_mode == "semantic_mask") {
            if (base_type == SHIP) {
                if (theme == 0) color_asset(asset_ptr, SHIP_0_COLOR, SHIP_0_COLOR, SHIP_0_COLOR);
                else if (theme == 1) color_asset(asset_ptr, SHIP_1_COLOR, SHIP_1_COLOR, SHIP_1_COLOR);
                else if (theme == 2) color_asset(asset_ptr, SHIP_2_COLOR, SHIP_2_COLOR, SHIP_2_COLOR);
                else if (theme == 3) color_asset(asset_ptr, SHIP_3_COLOR, SHIP_3_COLOR, SHIP_3_COLOR);
                else if (theme == 4) color_asset(asset_ptr, SHIP_4_COLOR, SHIP_4_COLOR, SHIP_4_COLOR);
                else if (theme == 5) color_asset(asset_ptr, SHIP_5_COLOR, SHIP_5_COLOR, SHIP_5_COLOR);
            }
            else if (base_type == PLAYER_BULLET)
                color_asset(asset_ptr, PLAYER_BULLET_COLOR, PLAYER_BULLET_COLOR, PLAYER_BULLET_COLOR);
            else if (base_type == PANEL)
                color_asset(asset_ptr, PANEL_COLOR, PANEL_COLOR, PANEL_COLOR);
            else if (base_type == TARGET_BACKGROUND)
                color_asset(asset_ptr, TARGET_BACKGROUND_COLOR, TARGET_BACKGROUND_COLOR, TARGET_BACKGROUND_COLOR);
            else if (base_type == EXPLOSION || base_type == EXPLOSION2 || base_type == EXPLOSION3 || base_type == EXPLOSION4 || base_type == EXPLOSION5)
                color_asset(asset_ptr, EXPLOSION_COLOR, EXPLOSION_COLOR, EXPLOSION_COLOR);
    ...
    void game_draw(QPainter &p, const QRect &rect) override {
        ...
        QColor juice_color, progress_color;
        if (options.vision_mode == "semantic_mask" || options.vision_mode == "fg_mask") {
            juice_color = QColor(JUICE_COLOR, JUICE_COLOR, JUICE_COLOR);
            progress_color = QColor(PROGRESS_COLOR, PROGRESS_COLOR, PROGRESS_COLOR);
        } else {
            juice_color = QColor(66, 245, 135);
            progress_color = QColor(245, 66, 144);
        }
    ...
    void game_reset() override {
        ...
        targets_hit = 0;
        target_quota = 20;
        max_possible_score = target_quota * POSITIVE_REWARD + COMPLETION_BONUS;
        spawn_prob = 0.06f;
