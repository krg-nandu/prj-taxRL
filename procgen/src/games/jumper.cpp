...
const int SPIKE_COLOR = 50;
const int CLOCK_COLOR = 150;
const int DISTANCE_BAR_COLOR = 175;
const int HIGHLIGHT_COLOR = 225;
  ...
  public:
    ...
    std::shared_ptr<QImage> segment_asset(std::shared_ptr<QImage> asset_ptr, int base_type, int theme) override {
        if (options.vision_mode == "semantic_mask") {
            if (base_type == PLAYER || base_type == PLAYER_JUMP || base_type == PLAYER_RIGHT1 || base_type == PLAYER_LEFT1 || base_type == PLAYER_RIGHT2 || base_type == PLAYER_LEFT2)
                color_asset(asset_ptr, PLAYER_COLOR, PLAYER_COLOR, PLAYER_COLOR);
            else if (base_type == SPIKE)
                color_asset(asset_ptr, SPIKE_COLOR, SPIKE_COLOR, SPIKE_COLOR);
            else if (base_type == GOAL)
                color_asset(asset_ptr, GOAL_COLOR, GOAL_COLOR, GOAL_COLOR);
            else if (base_type == CAVEWALL || base_type == CAVEWALL_TOP)
    ...
    void draw_compass(QPainter &p, const QRect &rect) {
        ...
        if (options.vision_mode == "semantic_mask" || options.vision_mode == "fg_mask")
            clock_color = QColor(CLOCK_COLOR, CLOCK_COLOR, CLOCK_COLOR);
        else
            clock_color = QColor(168, 166, 158);
        ...
        if (options.vision_mode == "semantic_mask" || options.vision_mode == "fg_mask")
            highlight_color = QColor(HIGHLIGHT_COLOR, HIGHLIGHT_COLOR, HIGHLIGHT_COLOR);
        else
            highlight_color = QColor(252, 186, 3);
        ...
        if (jump_delta < 0 && !has_support) {
            ...
            if (options.vision_mode == "semantic_mask" || options.vision_mode == "fg_mask")
                p.setBrush(QColor(DISTANCE_BAR_COLOR, DISTANCE_BAR_COLOR, DISTANCE_BAR_COLOR, 0)); // no alpha
            else
    ...
    void game_step() override {
        ...
        // don't use alpha decay effects when segmenting
        if (options.vision_mode != "semantic_mask" && options.vision_mode != "fg_mask") {
