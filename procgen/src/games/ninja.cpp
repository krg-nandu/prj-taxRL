...
const int BOMB_COLOR = 50;
const int CHARGE_COLOR = 150;
const int THROWING_STAR_COLOR = 175;
const int FIRE_COLOR = 225;
  ...
  public:
    ...
    std::shared_ptr<QImage> segment_asset(std::shared_ptr<QImage> asset_ptr, int base_type, int theme) override {
        if (options.vision_mode == "semantic_mask") {
            ...
            else if (base_type == PLAYER || base_type == PLAYER_JUMP || base_type == PLAYER_RIGHT1 || base_type == PLAYER_RIGHT2)
                color_asset(asset_ptr, PLAYER_COLOR, PLAYER_COLOR, PLAYER_COLOR);
            else if (base_type == BOMB)
                color_asset(asset_ptr, BOMB_COLOR, BOMB_COLOR, BOMB_COLOR);
            else if (base_type == THROWING_STAR)
                color_asset(asset_ptr, THROWING_STAR_COLOR, THROWING_STAR_COLOR, THROWING_STAR_COLOR);
            else if (base_type == FIRE)
                color_asset(asset_ptr, FIRE_COLOR, FIRE_COLOR, FIRE_COLOR);
    ...
    void game_draw(QPainter &p, const QRect &rect) override {
        ...
        if (options.vision_mode == "semantic_mask" || options.vision_mode == "fg_mask")
            charge_color = QColor(CHARGE_COLOR, CHARGE_COLOR, CHARGE_COLOR);
        else
            charge_color = QColor(66, 245, 135);
