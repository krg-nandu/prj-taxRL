...
const int ENEMY_COLOR = 150;
const int COIN_COLOR = GOAL_COLOR;
  ...
  public:
    ...
    std::shared_ptr<QImage> segment_asset(std::shared_ptr<QImage> asset_ptr, int base_type, int theme) override {
        if (options.vision_mode == "semantic_mask") {
            ...
            else if (base_type == WALL_TOP || base_type == WALL_MID)
            ...
            else if (base_type == COIN)
                color_asset(asset_ptr, COIN_COLOR, COIN_COLOR, COIN_COLOR);
    ...
    void game_reset() override {
        ...
        generate_platforms();
        max_possible_score = coin_quota * COIN_REWARD + COMPLETION_BONUS;
    }
